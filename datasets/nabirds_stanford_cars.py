...
class NABirdsStanfordCars(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, download=None, modified_params=None,
                 class_list=None):
        super(NABirdsStanfordCars, self).__init__(root, transform=transform, target_transform=target_transform)
        if download is True:
        ...
        sc_dataset = torchvision.datasets.StanfordCars(root=root.replace('nabirds', ''), split='train' if self.train else 'test', transform=transform)
        print()
...
if __name__ == '__main__':
    ...
    test_dataset = NABirdsStanfordCars('/media/nkondapa/SSD2/data/nabirds', train=False, download=False)
